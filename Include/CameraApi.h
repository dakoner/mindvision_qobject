...
/// @ingroup API_EXPOSURE
/// \~chinese
/// \brief 设置多重曝光的曝光延迟值。(此功能仅线阵相机支持)
/// \param [in] hCamera 相机的句柄。
/// \param [in] index 曝光索引。
/// \param [in] fExpDelay 要设定的曝光延迟值。
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
/// \~english
...
/// @ingroup API_EXPOSURE
/// \~chinese
/// \brief 获取多重曝光的曝光延迟值。(此功能仅线阵相机支持)
/// \param [in] hCamera 相机的句柄。
/// \param [in] index 曝光索引。
/// \param [out] pfExpDelay  指针，返回曝光延迟值。
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
...
/// @ingroup API_EXPOSURE
/// \~chinese
/// \brief 设置多重曝光的频闪脉冲宽度。(此功能仅线阵相机支持)
/// \param [in] hCamera 相机的句柄。
/// \param [in] index 曝光索引。
/// \param [in] fStrobeWidth 要设定的频闪脉冲宽度值。
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
/// \~english
...
/// @ingroup API_EXPOSURE
/// \~chinese
/// \brief 获取多重曝光的频闪脉冲宽度。(此功能仅线阵相机支持)
/// \param [in] hCamera 相机的句柄。
/// \param [in] index 曝光索引。
/// \param [out] pfStrobeWidth  指针，返回频闪脉冲宽度值。
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
...
/// @ingroup API_EXPOSURE
/// \~chinese
/// \brief 设置多重曝光的频闪输出选择。(此功能仅线阵相机支持)
/// \param [in] hCamera 相机的句柄。
/// \param [in] index 曝光索引。
/// \param [in] uStrobeOutSel 要设定的频闪输出选择值。
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
/// \~english
...
/// @ingroup API_EXPOSURE
/// \~chinese
/// \brief 获取多重曝光的频闪输出选择。(此功能仅线阵相机支持)
/// \param [in] hCamera 相机的句柄。
/// \param [in] index 曝光索引。
/// \param [out] puStrobeOutSel  指针，返回频闪输出选择值。
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
...
/// @ingroup API_EXPOSURE
/// \~chinese
/// \brief 获取多重曝光的功能支持位。(此功能仅线阵相机支持)
/// \param [in] hCamera 相机的句柄。
/// \param [out] pSupported 支持的功能。（bit0: gamma）
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
/// \~english
...
/// @ingroup API_EXPOSURE
/// \~chinese
/// \brief 获取多重曝光的混合功能支持位。(此功能仅线阵相机支持)
/// \param [in] hCamera 相机的句柄。
/// \param [out] pSupported 支持的混合个数。（bit0=1支持2个曝光的混合，bit1=1支持3个 。。。）
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
/// \~english
...
/// @ingroup API_EXPOSURE
/// \~chinese
/// \brief 获取多重曝光的混合功能是否启用
/// \param [in] hCamera 相机的句柄。
/// \param [out] pbEnable 返回启用状态
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
/// \~english
...
/// \brief 设置多重曝光混合的系数
/// \param [in] hCamera 相机的句柄。
/// \param [in] pRatios 系数指针（系数取值范围0-100，表示%0-%100）
/// \param [in] iNumRatios 系数个数（系数个数=曝光数量-1，比如3个曝光就只需配置2个系数，系数3=100-(系数1+系数2)）
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
/// \~english
...
/// \brief 获取多重曝光混合的系数
/// \param [in] hCamera 相机的句柄。
/// \param [out] pRatios 系数指针
/// \param [inout] piNumRatios 系数个数，输入：调用前设置为pRatios数组大小   输出：返回读取到的系数个数(曝光数量-1)
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
/// \~english
...
/// \return Returns CAMERA_STATUS_SUCCESS(0) successfully. Otherwise, it returns a non-zero error code. Please refer to the definition of the error code in CameraStatus.h.
MVSDK_API CameraSdkStatus __stdcall CameraFlatFieldingCorrectGetCoefficients(
	CameraHandle hCamera,
	float* pCoefficients,
	int* pNumCoefficient
	);
...
/// \return Returns CAMERA_STATUS_SUCCESS(0) successfully. Otherwise, it returns a non-zero error code. Please refer to the definition of the error code in CameraStatus.h.
MVSDK_API CameraSdkStatus __stdcall CameraFlatFieldingCorrectSetCoefficients(
	CameraHandle hCamera,
	float* pCoefficients,
	int iNumCoefficient
	);
...
/// @ingroup API_GRAB
/// \~chinese
/// \brief 提交抓拍请求，抓拍完成后图像数据会直接存到用户提供的缓冲区。
/// \param [in] hCamera 相机的句柄。
/// \param [inout] pUserBuffer 用来接收图像的缓冲区。
/// \param [inout] pUserBufferSize 缓冲区大小，如果大小不够则提交失败，并且返回当前配置下需要的最小缓冲区大小。如果pUserBuffer=NULL，也会返回需要的最小缓冲区大小。
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
/// \note 请求提交成功后需调用@link CameraWaitUserBufferSnap @endlink来完结请求。
/// \~english
...
/// \brief 取消已提交的抓拍请求
/// \param [in] hCamera 相机的句柄。
/// \param [in] pUserBuffer 抓拍请求使用的用户缓冲区。
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
/// \note 取消成功后依旧需要调用@link CameraWaitUserBufferSnap @endlink来完结请求。
/// \~english
...
/// \param [in] hCamera 相机的句柄。
/// \param [in] pUserBuffer 抓拍请求使用的用户缓冲区。
/// \param [out] pSnapStatus 返回抓拍状态，CAMERA_STATUS_SUCCESS表示抓拍成功，其他值表示抓拍错误。
/// \param [out] pFrameInfo 图像的帧头信息指针。
/// \param [out] ppImgStart 返回图像起始指针。
/// \param [in] wTimes 抓取图像的超时时间，单位毫秒。在wTimes时间内还未获得图像，则该函数会返回超时错误。
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
/// \note 由于图像数据可能额外包含一些头信息，因此图像数据起始位置可能不等于用户提供的缓冲区首地址，始终使用ppImgStart返回的地址来访问图像数据。
/// \~english
/// \brief Submit a capture request, and the image data will be directly stored in the buffer provided by the user after the capture is completed.
/// \param [in] hCamera Camera handle.
/// \param [in] pUserBuffer The user buffer used by the capture request.
